...
static void DefglobalRemoveCallback(
  ...
  XtPointer call_data)
  {
     ...
     SetCommandString(GetCurrentEnvironment(),"(undefglobal ");
     PrintRouter("wclips","(undefglobal ");
     AppendCommandString(GetCurrentEnvironment(),current->string);
...
static  void DefglobalPprintCallback(
  ...
  if(current->list_index == XAW_LIST_NONE ||
     GetDefglobalPPForm((struct constructHeader *) FindDefglobal(current->string)) == NULL)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdefglobal ");
...
static void AgendaFire(
  ...
  for (i = 0; i < current->list_index ; i++)
    act_ptr = (struct activation *) GetNextActivation(act_ptr);
  MoveActivationToTop(GetCurrentEnvironment(),(VOID*)act_ptr);
  MoveEndOfFile(dialog_text, &TheEvent);
  ...
  XtPointer call_data)
{
  if (Browse_status[FOCUS_WIN])
  ...
  else if(focus != NULL)
   {
     ...
     SetFocusChanged(GetCurrentEnvironment(),FALSE);
     PrintChangedFocus();
     XtSetArg(TheArgs[0], XtNleftBitmap,checker);
   }
  else
...
static void DefruleRemoveCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
    return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undefrule ");
...
static void DefruleMatchesCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(matches ");
...
static void DefrulePprintCallback(
  ...
  if(current->list_index == XAW_LIST_NONE ||
     GetDefrulePPForm((struct constructHeader *) FindDefrule(current->string)) == NULL)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdefrule ");
...
static void DefruleRefreshCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(refresh ");
...
static void DeffactsRemove(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undeffacts ");
...
static void DeffactsPprint(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdeffacts ");
...
static void DeftemplateRemove(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undeftemplate ");
...
static void DeftemplatePprint(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdeftemplate ");
...
static void DeffunctionRemoveCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undeffunction ");
...
static void DeffunctionPprintCallback(
  ...
  if(current->list_index == XAW_LIST_NONE ||
     GetDeffunctionPPForm(FindDeffunction(current->string)) == NULL)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdeffunction ");
...
static void DefgenericRemoveCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undefgeneric ");
...
static void DefgenericPprintCallback(
  ...
  if(current->list_index == XAW_LIST_NONE ||
     GetDefgenericPPForm(FindDefgeneric(current->string)) == NULL)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdefgeneric ");
  ...
  XtPointer call_data)
{
  ...
  defgenericPtr = FindDefgeneric(current->string);
  XtSetArg(TheArgs[0],XtNstate,GetDefgenericWatch(defgenericPtr));
...
static void RemoveDefmethodCallback(
  ...
  XtPointer call_data)
  {
    ...
    theIndex[i] = 0;
    MoveEndOfFile(dialog_text, &TheEvent);
    SetCommandString(GetCurrentEnvironment(),"(undefmethod  ");
    PrintCLIPS("wclips","(undefmethod ");
    AppendCommandString(GetCurrentEnvironment(),curr_def_name);
    PrintCLIPS("wclips",curr_def_name);
    AppendCommandString(GetCurrentEnvironment()," ");
    PrintCLIPS("wclips"," ");
    AppendCommandString(GetCurrentEnvironment(),theIndex);
    PrintCLIPS("wclips",theIndex);
    AppendCommandString(GetCurrentEnvironment(),")\n");
    PrintCLIPS("wclips",")\n");
  /* ================================================== */
  /* Set this flag to True to get out of the event loop */
...

{
    ...
    MoveEndOfFile(dialog_text, &TheEvent);
    AppendCommandString(GetCurrentEnvironment(),"(ppdefmethod ");
...

{
    ...
    theIndex[i] = 0;
    MethodIndex = (unsigned)atoi(theIndex);
    XtSetArg(TheArgs[0],XtNstate,GetDefmethodWatch(FindDefgeneric(curr_def_name),MethodIndex));
...
char *Aname)
  ...
  if(item_list1 != NULL)
   free(item_list1);
  if((theIndex = GetNextDefmethod(genrc_ptr,0)) == 0)
   {
     item_list1 = NULL;
     return(0);
   }
  ...
  while (theIndex != 0)
   {
     GetDefmethodDescription(buf,60,genrc_ptr,theIndex);
     ...
     theIndex = GetNextDefmethod(genrc_ptr,theIndex);
   }
...
static void DefinstancesRemoveCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undefinstances ");
...
static void DefinstancesPprintCallback(
  ...
  if(current->list_index == XAW_LIST_NONE ||
     GetDefinstancesPPForm(FindDefinstances(current->string)) == NULL)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdefinstances ");
...
static void DefclassRemoveCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(undefclass ");
...
static void DefclassDescribeCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(describe-class ");
...
static void DefclassBrowseCallback(
  ...
  if(current->list_index == XAW_LIST_NONE)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(browse-classes ");
...
static void DefclassPprintCallback(
  ...
  if(current->list_index == XAW_LIST_NONE ||
     GetDefclassPPForm(FindDefclass(current->string)) == NULL)
      return;
  MoveEndOfFile(dialog_text, &TheEvent);
  SetCommandString(GetCurrentEnvironment(),"(ppdefclass ");
  ...
  XtPointer call_data)
{
    ...
    buf[i] = 0;
    MoveEndOfFile(dialog_text, &TheEvent);
    AppendCommandString(GetCurrentEnvironment(),"(undefmessage-handler  ");
    PrintCLIPS("wclips","(undefmessage-handler  ");
    AppendCommandString(GetCurrentEnvironment(),curr_def_name);
    PrintCLIPS("wclips",curr_def_name);
    AppendCommandString(GetCurrentEnvironment()," ");
    PrintCLIPS("wclips"," ");
    AppendCommandString(GetCurrentEnvironment(),buf);
  ...
  XtPointer call_data)
{
    ...
    if(GetDefmessageHandlerPPForm(FindDefclass(curr_def_name),messageIndex) == NULL)
      return;
    MoveEndOfFile(dialog_text, &TheEvent);
    AppendCommandString(GetCurrentEnvironment(),"(ppdefmessage-handler  ");
...
static void DefruleMngrCheckboxesCallback(
  ...
  XtPointer call_data)
  {
     ...
     if(current->list_index == XAW_LIST_NONE)
            return;
     defrulePtr = FindDefrule(current->string);
     XtSetArg(TheArgs[0],XtNstate,DefruleHasBreakpoint(defrulePtr));
     XtSetValues(CheckBoxes[0],TheArgs,1);
     XtSetArg(TheArgs[0],XtNstate,GetDefruleWatchActivations(defrulePtr));
     XtSetValues(CheckBoxes[1],TheArgs,1);
     XtSetArg(TheArgs[0],XtNstate,GetDefruleWatchFirings(defrulePtr));
...
static void DefclssMngrChckbxCallback(
  ...
  XtPointer call_data)
  {
     ...
     if(current->list_index == XAW_LIST_NONE)
            return;
     defclassPtr = FindDefclass(current->string);
     XtSetArg(TheArgs[0],XtNstate,GetDefclassWatchInstances(defclassPtr));
     XtSetValues(CheckBoxes[0],TheArgs,1);
     XtSetArg(TheArgs[0],XtNstate,GetDefclassWatchSlots(defclassPtr));
     XtSetValues(CheckBoxes[1],TheArgs,1);
  ...
  XtPointer call_data)
{
  ...
  deftemplatePtr = FindDeftemplate(current->string);
  XtSetArg(TheArgs[0],XtNstate,GetDeftemplateWatch(deftemplatePtr));
  XtSetValues(checkbox,TheArgs,1); 
}
  ...
  XtPointer call_data)
{
  ...
  deffunctionPtr = FindDeffunction(current->string);
  XtSetArg(TheArgs[0],XtNstate,GetDeffunctionWatch(deffunctionPtr));
  XtSetValues(checkbox,TheArgs,1);
  ...
  XtPointer call_data)
{
 ...
 /* ======================================= */
  ...
  defclassPtr = FindDefclass(curr_def_name);
  theIndex = FindDefmessageHandler(defclassPtr,buf1,buf2);
  XtSetArg(TheArgs[0],XtNstate,GetDefmessageHandlerWatch(defclassPtr,theIndex));
  XtSetValues(checkbox,TheArgs,1);
  free(buf1);
}
